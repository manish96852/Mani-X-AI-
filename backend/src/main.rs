...
async fn main() -> std::io::Result<()> {
    ...
    let file_appender = tracing_appender::rolling::daily("./logs", "mani-x-ai.log");
    ...
    let filter = EnvFilter::new("mani_x_ai=trace");
...
mod test {
    ...
    async fn test_mint_burn_without_native_coin() -> Result<()> {
        ...
        let contract_address = config::MANIXAI_CONTRACT_ADDRESS;
    ...
    async fn test_rebalance() -> Result<()> {
        ...
        let vault_contract = ManiXAIVault::new(Address::from_str(contract_address)?, evm_provider);
